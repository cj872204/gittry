...
#define ABSDKVersionString @"1.0.1"
...
@section iOS-Audio-Errors A Note About kAudioSessionIncompatibleCategory Error
 
 There is currently an iOS bug, apparent on both iOS 5 and 6, that manifests in apps that use the PlayAndRecord
 audio session category under certain multitasking conditions. The issue is triggered reliably via the following
 steps:
 ...
 We strongly recommend writing code to recognize this error state, and inform the user accordingly,
 recommending that the user relaunches their apps.
 ...
 Apple have been made aware of the problem. In the meantime, please relaunch all your music apps,
 making sure you launch the Audiobus app first.
 </blockquote>
 
 **We've recently discovered a workaround that partially, but not entirely, addresses this problem:** 
 Deactivating and reactivating the audio session.
...
@page Recipes Common Recipes
 ...
 We also recommend that you keep your app running in the background even after it has been
 disconnected from Audiobus, as users may wish to disconnect your app, but then resume using it
 shortly afterwards. Suspending the app after a disconnection occurs may result in user
 confusion, so we advise against it.
 ...
 - (void)connectionsChanged:(NSNotification*)notification {
    ...
    if ( !_audioEngine.running && _audiobusController.connected ) {
        [_audioEngine start];
    }
 }
